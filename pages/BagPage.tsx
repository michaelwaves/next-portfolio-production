...
import { BagItems } from '@/components/models/BagItems'
import { Canvas } from '@react-three/fiber';
import { Suspense, useState } from 'react'
import { BagItemData } from '@/data/BagItemData'
...
export default function BagPage() {
    ...
    const bookComponents = BagItemData.map((book, i) => {
        return (
    ...
    const handleClick = (e: any) => {
        e.stopPropagation()
        dispatch(toggleState('bag'))
    }
    return (
        <motion.div
            exit={myExit}
            className="text-xl md:text-2xl fixed top-0 left-0 z-10 w-full h-full overflow-y-scroll md:scrollbar items-center justify-center bg-white dark:bg-black" >
            <div className='header-top'>
                <h1 className='w-2/3 text-5xl font-body title'>Bag Things</h1>
                    ...
                    >
                        <Canvas>
                            <Suspense fallback={
                                ...
                                <BagItems rotation-y={rotation} index={index} />
                            </Suspense>
                        </Canvas>
